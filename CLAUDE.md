...
4. **Social Security / pensions**: implemented as external cash flows on the scenario (`src/engine/cashflows.ts`): `incomes` (recurring real $/yr with start/end years) and `cashflows` (one-time, signed). Income offsets the portfolio draw — the withdrawal strategy still defines total spending — and surpluses are invested at the current sleeve mix; a year fully covered by income is never a depletion. Optional `retireAge` (presentation-only) lets timing be entered as ages. ✅ Implemented.  
